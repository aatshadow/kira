...
export default function MeetingsPage() {
  const { meetings, loading, editMeeting } = useMeetings()
  ...
  const handleEdit = (meeting: Meeting) => {
    openModal('meeting-edit', { meeting })
  }
  ...
  return (
    <div className="py-8">
      ...
      {meetings.length === 0 ? (
        <EmptyState
          icon={Calendar}
          title="Sin meetings registrados"
          description="Crea meetings manualmente o conecta Google Calendar para sincronizarlos"
          actionLabel="+ Nuevo meeting"
          onAction={() => openModal('meeting-create')}
        />
      ) : (
        <Tabs value={tab} onValueChange={(v) => setTab(v || 'upcoming')}>
          ...
          <TabsContent value="upcoming" className="space-y-2">
            {upcoming.length === 0 ? (
              <p className="text-sm text-muted-foreground py-4">No hay meetings próximos</p>
            ) : (
              upcoming.map((m) => (
                <MeetingCard
                  key={m.id}
          ...
          <TabsContent value="completed" className="space-y-2">
            {completed.length === 0 ? (
              <p className="text-sm text-muted-foreground py-4">No hay meetings completados</p>
            ) : (
              completed.map((m) => (
                <MeetingCard
                  key={m.id}
          ...
          <TabsContent value="cancelled" className="space-y-2">
            {cancelled.length === 0 ? (
              <p className="text-sm text-muted-foreground py-4">No hay meetings cancelados</p>
            ) : (
              cancelled.map((m) => (
                <MeetingCard
                  key={m.id}
